...
public:
  ...
  */
  ...
  //定义屏幕缓存buffer的结构体
  typedef struct{
    uint16_t *buffer1;/**<屏幕缓存buffer,屏幕的十分之一大小>**/
    uint16_t x1;/**<buffer屏幕上的x轴开始坐标>**/
    uint16_t y1;/**<buffer屏幕上的y轴开始坐标>**/
    uint16_t x2;/**<buffer屏幕上的x轴结束坐标>**/
    uint16_t y2;/**<buffer屏幕上的y轴结束坐标>**/
    uint8_t opt;/**<透明度>**/
  } sDisplay_t;
  //坐标系控件的样式
  typedef enum{
  ...
  //存储坐标数据的结构体
  struct sLine{
    sPoint_t *point;/**<坐标上图形的点的数据>**/
    uint8_t number;/**<图形点的个数>**/
    uint8_t uid;/**<图形的编号>**/
    uint16_t pointColor;/**<图形的颜色>**/
    struct sLine *line;/**<下一个图形的数据>**/
  };
 typedef struct sLine sLine_t;
  /*!
    结构体类型，用来存储坐标系控件的私有参数
  */
  struct coordinate:object{
    sLine_t lined;/**<坐标系内的图形存储位置的头>**/
    uint8_t lineNumber;/**<图形的个数>**/
    uint8_t intervalX;/**<>**/
    uint8_t intervalY;/**<>**/
    uint8_t intervalWidth;/**<>**/
    eCoordinateType_t type;/**<图形的类型>**/
    uint8_t pointRadius;/**<点的大小>**/
    uint16_t pointColor;/**<点的颜色>**/
    void setPoint(int16_t poi[][2],uint8_t number,uint16_t color);/**<在坐标系上加入一个新图形>**/
    void setType(eCoordinateType_t type);/**<选择图形的类型>**/
  };
...
private:
  ...
  void drawCoordLine(void * obj,sLine_t *line,sDisplay_t *disPlay = NULL);
  ...
  void drawCoordinatePart(void *obj);
  void drawRPart(sDisplay_t *disPlay,int16_t x,int16_t y,int16_t w,int16_t h,uint16_t color1,uint8_t opt = 255);
  void fillRoundRectP(int16_t x, int16_t y, int16_t w,int16_t h, int16_t r, uint16_t hcolor,uint16_t color,sDisplay_t *disPlay = NULL);
  void fillCircleHelperP(int16_t x0, int16_t y0, int16_t r,uint8_t corners, int16_t delta,uint16_t width1 ,uint16_t hcolor,uint16_t color,sDisplay_t *disPlay = NULL);
  ...
  uint16_t gcolor,uint16_t edgeColor,uint8_t shadow,uint16_t bg_color = 0xffff,sDisplay_t *disPlay = NULL);
  void fillCircleP(int16_t x0, int16_t y0, int16_t r,uint16_t mcolor,uint16_t color,sDisplay_t *disPlay = NULL);
  void drawLine(void *obj ,int16_t x0, int16_t y0, int16_t x1, int16_t y1,
        uint16_t color,sDisplay_t *disPlay = NULL);
  #endif
  sPoint_t * position; 
  DFRobot_GDL *_gdl;
  eTheme_t theme;
