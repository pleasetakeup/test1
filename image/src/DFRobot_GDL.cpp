...
void DFRobot_GDL::fillScreen(uint16_t color){
  ...
  pushColor(color,(uint16_t)_width*(uint16_t)_height);
}
...
void DFRobot_GDL::fillRect(int16_t x, int16_t y, uint16_t w, uint16_t h, uint16_t color){
  setDisplayArea((uint16_t)x, (uint16_t)y, w, h, color);
  uint32_t w1 = w;
  uint32_t h1 = h;
  pushColor(color,w1*h1);
}
...
void DFRobot_GDL::drawPicture(uint16_t x,uint16_t y,uint16_t w,uint16_t h,uint16_t *picture){
    setDisplayArea( x,  y,  w,  h, 0);
