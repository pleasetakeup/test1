...
DFRobot_UI::sCoordinate_t &DFRobot_UI::creatCoordinate() {
  ...
  coordData->lined.point = NULL;
  coordData->lined.line = NULL;
  //coordData->line = NULL;
  coordData->lined.number = 0;
  coordData->lined.uid = 0;

  coordData->intervalWidth = 2;
  coordData->type = TCOLUMN;
...
void DFRobot_UI::sCoordinate_t::setPoint(int16_t poi[][2], uint8_t number, uint16_t color) {
  ...
  sLine_t* obj = &this->lined;
  for (uint8_t i = 0 ; i < this->lineNumber - 1; i++) {
    obj = obj->line;
  }
  obj->line =  lineData;
}
void DFRobot_UI::sCoordinate_t::setType(eCoordinateType_t type) {
...
void DFRobot_UI::drawCoordinate(void *obj)
{
  ...
  _gdl->setRotation(1);
#if defined(ESP32) || defined(ESP8266) ||  defined(ARDUINO_SAM_ZERO)||(__AVR_ATmega2560__)
  drawCoordinatePart(coord);
#else
  _gdl->drawRoundRect(coord->posx - 1, coord->posy - coord->height - 1, coord->width + 2, coord->height + 2, 6, 0x4219);
  _gdl->fillRoundRect(coord->posx, coord->posy - coord->height, coord->width, coord->height, 4, coord->lvColor);
  ...
  _gdl->print("y");
  sLine_t *line = coord->lined.line;
  for (uint8_t i = 0 ; i < coord->lineNumber; i++) {
...
#if defined(ESP32) || defined(ESP8266) ||  defined(ARDUINO_SAM_ZERO)||(__AVR_ATmega2560__)
void DFRobot_UI::drawCoordinatePart(void *obj) {

  sCoordinate_t *coord = (sCoordinate_t *)obj;
  sDisplay_t *disPlay = (sDisplay_t *)malloc(sizeof(sDisplay_t));
  uint16_t lenght = 2400;
  disPlay->buffer1 = color2;
  uint16_t width , height;
  width =  coord->width;
  height = 2400 / coord->width;
  uint8_t intx = coord->width / coord->intervalX;
  uint8_t inty = coord->height / coord->intervalY;
  disPlay->opt = 100;
  disPlay->x1 = coord->posx;
  disPlay->y1 = coord->posy - coord->height;
  disPlay->x2 = coord->posx + coord->width;
  disPlay->y2 = disPlay->y1 + height;
  for (uint16_t i = 0 ; i < 2400; i++) {
    disPlay->buffer1[i] = bgColor;
  }
  for (uint16_t i = coord->posy - coord->height; i < coord->posy; i += height) {

    if (disPlay->y2 > (disPlay->y1 + coord->height)) disPlay->y2 = coord->posy;
    drawRectP(coord->posx, coord->posy - coord->height, coord->width, coord->height, 4, coord->lvColor, coord->lvColor, 0x4219, 1, bgColor, disPlay);
    for (uint8_t j = 1; j < coord->intervalX ; j += 1 ) {
      drawRPart(disPlay, coord->posx + intx * j, coord->posy - coord->height, 1, coord->height, coord->bgColor);
    }
    for (uint8_t j = 1; j < coord->intervalY; j += 1) {
      drawRPart(disPlay, coord->posx, (coord->posy - coord->height) + inty * j, coord->width, 1, coord->bgColor);

    }

    sLine_t *line = coord->lined.line;
    for (uint8_t j = 0 ; j < coord->lineNumber; j++) {
      drawCoordLine(coord, line, disPlay);
      line = line->line;


    }
    _gdl->drawPicture(disPlay->x1, disPlay->y1, width, disPlay->y2 - disPlay->y1 , disPlay->buffer1);
    for (uint16_t j = 0 ; j < 2400; j++) {
      disPlay->buffer1[j] = bgColor;
    }
    disPlay->y2 +=  height;
    disPlay->y1 += height;
  }
...
void DFRobot_UI::drawRPart(sDisplay_t *disPlay, int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color1, uint8_t opt ) {
  int16_t relativeX ;
  int16_t relativeY ;
  if ((x + w < disPlay->x1 || x > disPlay->x2) && (y + h < disPlay->y1 || y > disPlay->y2)) {
    return;
  }
  int16_t x1 = x >= disPlay->x1 ? x : disPlay->x1;
  int16_t y1 = y >= disPlay->y1 ? y : disPlay->y1;
  int16_t x2 = x + w <= disPlay->x2 ? x + w : disPlay->x2;
  int16_t y2 = y + h <= disPlay->y2 ? y + h : disPlay->y2;
  if (x1 > x2 || y1 > y2)  return;
  relativeX = x1 - disPlay->x1;
  relativeY = y1 - disPlay->y1;
  sUicolor_t bg_tmp ;
  bg_tmp.full = 0xffff;
  sUicolor_t color ;
  color.full = color1;
  sUicolor_t opa_tmp = colorMix(color, bg_tmp, opt);
  int16_t width1  = x2 - x1;
  int16_t height1 = y2 - y1;
  for ( int16_t i = 0 ;  i < height1 ; i++) {
    for (int16_t j = 0; j < width1; j++) {
      if (disPlay->buffer1[(i + relativeY) * (disPlay->x2 - disPlay->x1) + j + relativeX] != bg_tmp.full) {
        bg_tmp.full  = disPlay->buffer1[(i + relativeY) * (disPlay->x2 - disPlay->x1) + j + relativeX];
        opa_tmp = colorMix(color, bg_tmp, opt);
      }
      disPlay->buffer1[(i + relativeY) * (disPlay->x2 - disPlay->x1) + j + relativeX] = opa_tmp.full;
    }
  }
}
#endif
void DFRobot_UI::drawCoordLine(void * obj, sLine_t *line, sDisplay_t *disPlay) {
  sCoordinate_t *coord = (sCoordinate_t *)obj;

  for (uint8_t i = 0 ; i < line->number ; i++) {
    int16_t x = line->point[i].x;
    int16_t y = coord->height - line->point[i].y;
    if (coord->type == TLINE) {
#if defined(ESP32) || defined(ESP8266) ||  defined(ARDUINO_SAM_ZERO)||(__AVR_ATmega2560__)
      fillCircleP(x + 10, y + 10, 3, line->pointColor, line->pointColor, disPlay);
#else
      _gdl->fillCircle(x + 10, y + 10, 3, line->pointColor);
#endif
    }
  }
  for (uint8_t i = 1 ; i < line->number ; i++) {
    int16_t x = line->point[i].x ;
    int16_t y = coord->height - line->point[i].y ;
    if (x > coord->width) x =  coord->width;
#if defined(ESP32) || defined(ESP8266) ||  defined(ARDUINO_SAM_ZERO)||(__AVR_ATmega2560__)
     uint8_t offset =1;
     if(i == 1){ 
        offset =0;
      }
      drawLine(obj, x + 10, y + 10, line->point[i - 1].x + 10+offset, (coord->height - line->point[i - 1].y) + 10, line->pointColor, disPlay);

#else
    _gdl->drawLine(x + 10, y + 10, line->point[i - 1].x + 10, (coord->height - line->point[i - 1].y) + 10, line->pointColor);
#endif
  }
...
void DFRobot_UI::refreshSliser(void *obj)
{
  ...
  if (slider->change == 1) {
    ...
    if (position[0].x < slider->posx || (position[0].x > slider->posx + slider->width) || ((position[0].y < slider_y) || (position[0].y > slider_y + slider->sliderHeight))) {
      ...
      uint8_t offset = 0;
#endif
      _gdl->fillRect(slider->posx + slider->width / 2, slider->posy + 30 - offset, slider->fontSize * fontWidth * 3, slider->fontSize * fontHight, bgColor);
      _gdl->setCursor(slider->posx + slider->width / 2, slider->posy + 30);
...
#if defined(ESP32) || defined(ESP8266) ||  defined(ARDUINO_SAM_ZERO) ||(__AVR_ATmega2560__)
void DFRobot_UI::fillRoundRectP(int16_t x, int16_t y, int16_t w, int16_t h, int16_t r, uint16_t hcolor, uint16_t color, sDisplay_t *disPlay ) {
  ...
  for (uint16_t row = y + r; row <= y + h - r; row++) {
    ...
    if (disPlay == NULL)
      ...
      drawRPart(disPlay, x, posy, w, 1, actColor.full);
  ...
  if (disPlay == NULL) {
    ...
    fillCircleHelperP(x + r, y + r, r, 1, w - 2 * r - 1, h - 2 * r, hcolor, color, disPlay);
    fillCircleHelperP(x + r , y + r, r, 2, w - 2 * r - 1, h - 2 * r, hcolor, color, disPlay);
  }
}
void DFRobot_UI::fillCircleHelperP(int16_t x0, int16_t y0, int16_t r, uint8_t corners, int16_t delta,
                                   uint16_t height, uint16_t hcolor, uint16_t color, sDisplay_t *disPlay ) {
    ...
    if (x < (y + 1)) {
      if (corners & 1) {
        ...
        if (disPlay == NULL)
          ...
          drawRPart(disPlay, x0 - y, y1, 2 * y + delta, 1, actColor.full);
        }
      }
      if (corners & 2) {
        ...
        if (disPlay == NULL)
          ...
          drawRPart(disPlay, x0 - y, y0 - x, 2 * y + delta, 1, actColor.full);
        }
      }
    }
    if (y != py) {
      ...
      if (corners & 1) {
        ...
        if (disPlay == NULL)
          ...
          drawRPart(disPlay, x0 - px, y1, 2 * px + delta, 1, actColor.full);
        }
      }
      if (corners & 2) {
        ...
        if (disPlay == NULL)
          ...
          drawRPart(disPlay, x0 - px, y0 - py, 2 * px + delta, 1, actColor.full);
        }
      }
      py = y;
    }
...
void DFRobot_UI::drawRectP(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t radius, uint16_t mcolor,
                           uint16_t gcolor, uint16_t edgeColor, uint8_t shadow, uint16_t bg_Color, sDisplay_t *disPlay) {
  ...
  if (disPlay == NULL) {
    ...
    fillRoundRectP(x, y, width, height, radius, mcolor , gcolor, disPlay);
  }
  if (edgeColor != 0xffff) {
...
void DFRobot_UI::fillCircleP(int16_t x0, int16_t y0, int16_t r, uint16_t mcolor, uint16_t gcolor, sDisplay_t *disPlay ) {
  ...
  if (disPlay == NULL) {
    ...
    drawRPart(disPlay, x0 - r, y0, 2 * r + 1, 1, actColor.full);
    fillCircleHelperP(x0, y0, r, 3, 0, 0, mcolor, gcolor, disPlay);
  }
}
void DFRobot_UI::drawLine(void * obj, int16_t x0, int16_t y0, int16_t x1, int16_t y1,
                          uint16_t color, sDisplay_t *disPlay ) {
  sCoordinate_t * coord = (sCoordinate_t *)obj;
  ...
  for (; x0 <= x1; x0++) {
    if (steep) {
      if (disPlay == NULL)
        ...
        if (coord->type == TLINE)
          drawRPart(disPlay, y0, x0, 1, 1, color, disPlay->opt);
        else {
          if (y0 != temx) {
            drawRPart(disPlay, y0, x0, 1, coord->posy - x0, color, disPlay->opt);
          }
        }
      }
      temx = y0;
      ...
      if (disPlay == NULL) {
        ...
        if (coord->type == TLINE)
          drawRPart(disPlay, x0, y0, 1, 1, color, disPlay->opt);
        else {
          if (x0 != temx) {
            drawRPart(disPlay, x0, y0, 1, coord->posy - y0, color, disPlay->opt);
          }
        }
      }
      temx = x0;
