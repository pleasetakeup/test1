...
public:
   ...
   * @param h  图片的数组指针
   */
  void drawPicture(uint16_t x,uint16_t y,uint16_t w,uint16_t h,uint16_t *picture);
protected:
  /**
  ...
  virtual   void pushColor(uint16_t color,uint32_t size){};
...
protected:
  ...
  void pushColor(uint16_t color,uint32_t size);
};
