...
void DFRobot_UI::drawText(void *obj)
{
  ...
  if (theme == MODERN) {
    ...
    // _gdl->fillRoundRectP(btn->posx, btn->posy, btn->width, btn->height,6,btn->lvColor,btn->bgColor);
    // _gdl->drawRoundRect(btn->posx-1, btn->posy-1, btn->width+2, btn->height+2,6+2,0x8c5);
    // _gdl->drawRoundRect(btn->posx-2, btn->posy-2, btn->width+4, btn->height+4,6+4,0x8c5);
    
    _gdl->drawRectShadow(tb->posx-2, tb->posy-2-2, tb->width+4,tb->height+4, 10 ,0x4208 ,bgColor);
    
    _gdl->fillRoundRectP(tb->posx, tb->posy, tb->width,tb->height, 6,0xffff ,tb->lvColor);
    _gdl->drawRoundRect(tb->posx-1, tb->posy-1, tb->width+2,tb->height+2, 8,tb->lvColor);
    _gdl->drawRoundRect(tb->posx-2, tb->posy-2, tb->width+4,tb->height+4, 10,tb->lvColor);
    #else
  ...
  for (uint8_t i = 0 ; i < totalWord; i++) {
    _gdl->drawChar(offset_x + tb->posx + 13 * tb->fontSize * column, offset_y + tb->posy + 16 * tb->fontSize * line +5, tb->text[i] , LIGHTGREY_RGB565, tb->bgColor, tb->fontSize);
    column++;
    if (column >= tb->width / (13 * tb->fontSize) - 1) {
      column = 0 ;
      line++;
    }
    if (line > tb->height / (16 * tb->fontSize) - 1) return;
    tb->cursorx = column;
    tb->cursory = line;
  }
}
...
void DFRobot_UI::refreshTextBox(void *obj)
{
  ...
  uint8_t singleWord = tb->width / (13 * tb->fontSize) -1;
  if (tb->height/(13* tb->fontSize)<2){
      totalWord = singleWord-2;
  }
  else{
       totalWord = singleWord *(tb->height/(16* tb->fontSize)-1)-2;
  }
  offset_x = 5;
  offset_y = 5 ;
  ...
  if(position[0].x > tb->posx && position[0].x < tb->posx +tb->width && position[0].y > tb->posy && position[0].y < tb->posy + tb->height){
     ...
     cursorPosx = tb->posx + offset_x + ((tb->fontSize) * 13) * (tb->cursorx) ;
     cursorPosy = tb->posy + offset_y + (tb->fontSize) * 16 * (tb->cursory);
  }
 if(cursorPosx >= tb->posx && cursorPosx <= tb->posx +tb->width && cursorPosy >= tb->posy && cursorPosy <= tb->posy + tb->height){
  ...
  if (tb->state == DRAWTEXT) {
    if (theme == CLASSIC ) {
      _gdl->fillRoundRect(tb->posx, tb->posy, tb->width, tb->height, 13, tb->bgColor);
    }
    else {
      _gdl->fillRect(tb->posx, tb->posy, tb->width, tb->height, tb->bgColor);
    }
  ...
  if (tb->state == DRAWTEXT) {
    ...
    for (uint8_t i = 0 ; i < totalWord; i++) {
        _gdl->setCursor(offset_x + tb->posx + 13 * tb->fontSize * column,offset_y + tb->posy + 16 * tb->fontSize * line+8);
	    _gdl->setTextColor(0xffff);
    ...
    tb->cursorx = strlen(tb->text) % singleWord;
    tb->cursory = strlen(tb->text) / singleWord;
    _gdl->fillRect(tb->cursorx * (13 * tb->fontSize) + offset_x + tb->posx, tb->cursory * (16* tb->fontSize) + offset_y + tb->posy, 13* tb->fontSize, 13 * tb->fontSize, tb->bgColor);
    tb->state = NOCHANGE;
    if (line > tb->height / (13 * tb->fontSize) - 1) return;
    ...
    tb->text[strlen(tb->text)] =  tb->cache;
        _gdl->setCursor(tb->cursorx * (13 * tb->fontSize) + offset_x + tb->posx,tb->cursory * (16 * tb->fontSize)+8 + offset_y + tb->posy);
	    _gdl->setTextColor(0xffff);
        _gdl->print(tb->cache);
  ...
  if (theme == MODERN) {
    ...
    _gdl->fillRoundRect(btn->posx, btn->posy, btn->width, btn->height, 10, btn->bgColor);
        ...
        _gdl->drawRoundRect(btn->posx-1, btn->posy-1, btn->width+2, btn->height+2,6+2,0x8c5);
         _gdl->drawRoundRect(btn->posx-2, btn->posy-2, btn->width+4, btn->height+4,6+4,0x8c5);
        #else
        _gdl->fillRect(btn->posx, btn->posy, btn->width, btn->height,btn->fgColor);
        #endif
...
void DFRobot_UI::drawClickButton(sObject_t *obj,char *text , bool click)
{
    if (theme == MODERN) {
        ...
        #if defined(ESP32) || defined(ESP8266)
        _gdl->fillRoundRectP(obj->posx, obj->posy, obj->width, obj->height,6,obj->bgColor,obj->lvColor);
        _gdl->drawRoundRect(obj->posx-1, obj->posy-1, obj->width+2, obj->height+2,6+2,0x8c5);
         _gdl->drawRoundRect(obj->posx-2, obj->posy-2, obj->width+4, obj->height+4,6+4,0x8c5);
        #else
...
void DFRobot_UI::drawCursor(sTextBox_t* obj, uint8_t offset_x, uint8_t offset_y, bool state)
{
  ...
  uint16_t  x = obj->posx + offset_x + ((obj->fontSize) * 13) * (obj->cursorx) + 2;
  uint16_t  y = obj->posy + offset_y + (obj->fontSize) * 16 * (obj->cursory);
  uint16_t color ;
  if (state == 1 ) {
    color = obj->bgColor;
  }
  ...
  _gdl->fillRect(x, y-4, 2, obj->fontSize * 16, color);
...
void DFRobot_UI::drawButtonString(sObject_t *obj, eLocation_t x, eLocation_t y, char * c,bool click)
{
  ...
  _gdl->setFont(&FreeSans9pt7b);//设置字体为FreeMono12pt7b
  char b ;
  uint8_t length = (obj->width) / (14 * obj->fontSize);
  if (length >= strlen(c)) length = strlen(c);
  if (x == CENTER && y == CENTER) {

    po_x =  obj->posx + obj->width / 2 - (14 * obj->fontSize) / 2 ;
    po_y =  obj->posy + obj->height / 2+4;
    po_x = po_x - obj->fontSize * 14 * (strlen(c) - 1) * (0.5);
    if(obj->posx >= po_x)  po_x = obj->posx+4;
  }
  else if (x == LEFT && y == CENTER) {
    po_x =  obj->posx ;

    po_y =  obj->posy + obj->height / 2 - (14 * obj->fontSize) / 2 ;
  }

  for (uint16_t i = 0 ; i < length ; i++) {
    b = c[i];
    if (click == 0) {
        _gdl->setCursor(/*x=*/po_x + i * 14 * obj->fontSize,po_y);
	    _gdl->setTextColor(0xffff);
        _gdl->print(b);
